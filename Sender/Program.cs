        ...
        using (IModel model = con.CreateModel())
        {
            ...
            using (var reader = new StreamReader("D:\\DSProject\\Sender\\sensor.csv"))
            {
                ...
                while ((line = reader.ReadLine()) != null)
                {
                    ...
                    if (randomNumber % 2 == 0)
                    {
                         jsonMessage = $"{{\"timestamp\":\"{timestamp}\",\"DeviceId\":{deviceId},\"measurementValue\":{measurementValue}}}";
                    }
                    else
                    {
                         jsonMessage = $"{{\"timestamp\":\"{timestamp}\",\"DeviceId\":{deviceId2},\"measurementValue\":{measurementValue}}}";
                    }
